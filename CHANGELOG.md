...
- `ContradictionGuard`: each encoded clause value now records `value_provenance` (`parsed_from_text` | `caller_asserted`) in its trace step, and a caller-asserted count is noted in the result message — Z3 "consistency" over caller-invented numbers is disclosed, not silently presented as text-evidenced (#42).
- `ContradictionGuard`: value provenance is bound to the recognized constraint phrase's numeric operand — a number elsewhere in the clause text (e.g. a notice period in a term clause) no longer evidences the encoded value; keyword phrases without an adjacent number stay unmodeled (PR #45 review).
- `ContradictionGuard`: an UNSAT result involving caller-asserted values is `unverifiable` (non-proof conclusion) — the conflict proves only that invented numbers clash, not a contradiction of the text (PR #45 review).
...
- `ContradictionGuard`: a SAT result over caller-asserted clause values is `partial_coverage` with `verified=False` — never presented as a verified consistency proof; caller-asserted constraint steps carry `INFERRED` evidence (`is_proven()` is `False`) (PR #45 review).
